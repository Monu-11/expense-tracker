...
const Settings=(props)=>{
    ...
    const [budget,setBudget]=useState(budgetAmount.amount?budgetAmount.amount:'0')
    const err={}
    const [errors,setErrors]=useState({})
    const allCategories=useSelector(state=>state.categories)
    const validCategories=allCategories.filter((ele)=>ele.name!=='uncategorized')
    ...
    const handleBudgetSubmit=(e)=>{
        ...
        else{console.log('inside else condition')
            setErrors({})
            ...
            <form onSubmit={handleBudgetSubmit}>
                ...
                <div className='row'>
                    <div className='col-md-6'>
                        <input type='string' className='form-control' value={budget} name='budget' onChange={handleChange} 
                        placeholder='Enter your budget in rupees' disabled={budgetAmount.amount>0}/>
                        {errors&&<p style={{color:'red'}}>{errors.budgetError}</p>}
                    </div>
                    <div className='col-md-3'>
                     <input type='submit' value='Update' className='btn btn-primary budgetBtn'disabled={budgetAmount.amount>0} 
                         />
