...
const calculatePercentage=(num)=>{
    ...
    return res
     ...
     options = {
        ...
        data: [{
            ...
            indexLabel: "{name}: {y}",
            yValueFormatString: "#,###'%'",
            dataPoints: Object.entries(categoriesAmount).map((categoryAmount,index)=>{
                return { name:categoryAmount[0] , y:calculatePercentage(categoryAmount[1]) }
            })
        }],
       height:300,
       width:500
    ...
    return (
        <div className='block_container2 '> 
            <center><h3>Category wise split</h3><hr style={{backgroundColor:'black'}}/></center>
